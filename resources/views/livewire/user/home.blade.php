<div class="bg-white min-h-screen">    
    ...
    <section id="koleksi" class="py-12 px-8">
        ...
        <a href="{{ route('allBooks') }}" class="text-sm text-blue-700 flex justify-end px-4 py-2 rounded-lg hover:underline transition font-medium">Lihat selengkapnya -></a>
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-5">
            @foreach ($latestBooks as $book)
            <div class="group cursor-pointer" onclick="window.location='{{ route('books.show', $book->id) }}'">
                <div class="aspect-[2/3] rounded-xl overflow-hidden bg-blue-100 mb-3 flex items-end p-1 transition group-hover:shadow-lg">
                    @if ($book->cover)
                        <img src="{{ asset('storage/' . $book->cover) }}" alt="{{ $book->judul }}" class="w-full h-full object-cover rounded-xl group-hover:scale-105 transition duration-500">
                    @else
                        <span class="text-xs font-medium text-blue-800 leading-snug">{{ $book->judul }}</span>
                    @endif
                </div>
                <p class="text-sm font-medium text-gray-800 leading-snug group-hover:text-blue-700 transition">{{ $book->judul }}</p>
    ...
    {{-- Rekomendasi --}}
    <section id="rekomendasi" class="py-12 px-8 bg-gray-50">
        <div class="mb-7">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="bg-white border border-gray-100 rounded-2xl p-4 flex gap-4 hover:border-blue-200 transition cursor-pointer hover:shadow-md" onclick="window.location='{{ route('books.show', $book->id) }}'">
                <div class="w-14 h-20 rounded-lg bg-blue-100 flex-shrink-0 flex items-end p-1 overflow-hidden">
                    @if ($book->cover)
                        <img src="{{ asset('storage/' . $book->cover) }}" alt="{{ $book->judul }}" class="w-full h-full object-cover rounded shadow-sm">
                    @else
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-sm font-medium text-gray-800 leading-snug line-clamp-1 h-5 group-hover:text-blue-700 transition">{{ $book->judul }}</p>
                    <p class="text-xs text-gray-500 mt-0.5 mb-1">{{ $book->penulis }}</p>
                    <p class="text-xs text-gray-400 leading-relaxed line-clamp-2 h-8">{{ $book->deskripsi }}</p>
                </div>
            </div>
            @endforeach
        </div>
